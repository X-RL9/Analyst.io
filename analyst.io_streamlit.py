...
UPDATE: PDF uploads can now run Comparable Company Analysis. Since a PDF's
financial statements never contain sector/industry classification, the
user picks one from a dropdown (industry_options.PDF_INDUSTRY_OPTIONS),
and that gets passed through to run_pipeline as manual_sector/
manual_industry -- data_layer.py attaches it to the financials dict
exactly as if it had come from yfinance, so find_peers() works for both
the DCF beta-fallback (which needs it even if comps isn't selected) and
the comps mode itself.
...
it's read silently via st.secrets.
...
pdf_extraction.py, pipeline_real.py, industry_options.py, requirements.txt
"""
...
from industry_options import PDF_INDUSTRY_OPTIONS


def format_currency(value: float) -> str:
    """Auto-scales to billions/millions correctly regardless of company size."""
    abs_value = abs(value)
    if abs_value >= 1e9:
...
with st.sidebar:
    ...
    manual_sector, manual_industry = None, None
    ...
    if input_type_choice == "Ticker":
        ...
        industry_choice = st.selectbox(
            ...
            list(PDF_INDUSTRY_OPTIONS.keys()),
            help="A PDF's financial statements don't contain sector/industry "
                 "classification -- pick the closest match so Comparable "
                 "Company Analysis and the DCF's beta calculation can find peers.",
        )
        manual_sector, manual_industry = PDF_INDUSTRY_OPTIONS[industry_choice]
            ...
            try:
                if input_type_choice == "Ticker":
                    ...
                    results = run_pipeline(
                        ...
                        manual_sector=manual_sector, manual_industry=manual_industry,
                    )
